...
{
	m_ulKey = ulKey; 
	m_pCvgCpMgr = dynamic_cast<CCvgCpMgr*>(pCpMgr);
}
...
int CDeliverMgr::HandleOrder(CSamplerPacket& oOrder)
{
	try
	{
		...
		if (0 != m_pCvgCpMgr)
			return m_pCvgCpMgr->Forward(oPktRsp,m_ulKey);
        ...
        */
		...
		if( !mapSubscri.empty())
		{
			for(map<unsigned int,SUB_CONTEXT>::iterator it = mapSubscri.begin();it != mapSubscri.end();++it)
			{
				...
				if (0 != m_pCvgCpMgr)
				{
					int nRtn = m_pCvgCpMgr->Forward(oPkt,m_ulKey);
					if (0 > nRtn && nRtn != -2)
					{//检查到已经异常中断或未取消订阅的接口 
...
int CDeliverMgr::HandleFirstQuotation(string sKey,unsigned int uiNodeID)
{
	try
	{
		...
		if (0 != m_pCvgCpMgr)
		{
			m_pCvgCpMgr->Forward(oPkt,m_ulKey);
			m_uiOrderFwdCount++;
