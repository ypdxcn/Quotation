...
int CQuotationTbl::GetInstKeySet(char cMode,const vector<unsigned int>& vCnd, unsigned int uiNodeID, multimap<string,unsigned int>& mKeySet)
{
	...
	else
	{
		if (cMode == '0')
		{
			for (vector<unsigned int>::const_iterator itCnd = vCnd.begin(); itCnd != vCnd.end(); ++itCnd)
			{
				for (map<string,QUOTATION*>::iterator itQuo = m_mapQuotation.begin(); itQuo !=  m_mapQuotation.end(); ++itQuo)		
				{
					if (*itCnd == (*itQuo).second->m_CodeInfo.m_usMarketType)
					{
						mKeySet.insert(pair<string,unsigned int>((*itQuo).first, uiNodeID));
						nRtn = 0;
					}
				}
			}
		...
		else if (cMode == '1')
		{
			for (map<string,QUOTATION*>::iterator itQuo = m_mapQuotation.begin(); itQuo !=  m_mapQuotation.end(); ++itQuo)			
			{
				...
				for (vector<unsigned int>::const_iterator itCnd = vCnd.begin(); itCnd != vCnd.end(); ++itCnd)
				{
					if ((*itQuo).second->m_CodeInfo.m_usMarketType == *itCnd)
					{
						blFind = true;
						break;
					}
...
void CQuotationTbl::GetRecs(map<string,QUOTATION> & mapQuotation)
{
	CGessGuard guard(m_mutexTbl);

	for (map<string,QUOTATION*>::iterator it = m_mapQuotation.begin(); it != m_mapQuotation.end(); ++it)
	{
		mapQuotation.insert(pair<string,QUOTATION>((*it).first, *((*it).second)));
	}
}